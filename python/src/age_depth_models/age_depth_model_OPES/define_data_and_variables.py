...
def read_data(data):
    ...
    elif data.lower() == "dayu13a":
        df = pd.read_csv(
            os.path.join(base_path, "inputdata_251013A/Dayu cave.txt"), sep="\t"
        )
        D18O_timeseries = pd.read_csv(
            os.path.join(base_path, "inputdata_251013A/d18O_timeseries.txt"), sep="\t"
        )
...
def get_hmc_config(find_min = False, bias = "", cap = False, temp = False, umbrella = False):
    keys = ["N", "H", "dc", "cs", "dt", "ns", "co", "ndt", "nHMC", "nch", "a", "b", "ces", "cw", "s", "dE", "d", "sb", "eb", "nl", "g", "thr", "sbt", "ebt", "nt", "nlsp", "mi", "gl", "npc", "hmc", "hmcns", "ut", "rsp", "sd", "temps_unb", "temps_anders", "bt"]
    ...
    if not find_min:
        ...
        config["co"] = 10                                                                     # Cutout
        config["ndt"] = 1000                                                                    # Number of Leapfrog steps
