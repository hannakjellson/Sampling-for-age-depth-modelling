...
double bias_potential(int num_temps, double *betas, double d18o_energy, double *delta_F)
{
    ...
    {
        temp_term = -(betas[k] - 1) * d18o_energy;
        ...
        {
            sum_for_V += delta_F[k] * exp(temp_term - max_temp_term);
        }
    }
...
void grad_bias(int N, int num_temps, double *betas, double *variables, double d18o_energy, double *d18o_gradient, double *delta_F, double *bias_gradient)
{
    ...
    for (i = 0; i < num_temps; i++)
    {
        ...
        temp_term = -temp_factor * d18o_energy;
        if (temp_term > max_temp_term)
        {
        ...
        {
            exp_temp = delta_F[i] * exp(temp_term - max_temp_term);
            sum_for_V += exp_temp;
            for (l = 0; l < N; l++)
            {
...
void update_delta_F(int num_temps, double *betas, double energy, double *delta_F, double bias, double *df_out, int idx)
{
    int i;
    double temp_term;
    for (i = 0; i < num_temps; i++)
