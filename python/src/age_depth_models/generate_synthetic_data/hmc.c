...
#pragma omp parallel num_threads(hmcc->nch)
    {
        ...
        for (j = 0; j < hmcc->ns; j++)
        {
            ...
            energy_old = energy_function(d, c14_expected_ages, variables);

            // Compute gradient at old state
            grad_energy_function(d, c14_depth_indices, c14_expected_ages, variables, gradient);
            ...
            energy_new = energy_function(d, c14_expected_ages, variables);
