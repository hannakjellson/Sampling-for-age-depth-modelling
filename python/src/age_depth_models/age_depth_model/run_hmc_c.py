...
def main():
    ...
    lib.hmc(
        ...
        ctypes.c_int(config["L"]),
        ctypes.c_int(config["num_MH"]),
    ...
    samples = np.ctypeslib.as_array(
        samples_out, shape=(config["num_chains"], config["num_MH"], config["N"])
    )
    valid_samples = samples[:, int(config["num_MH"] / 2) :, :]
    np.save("../../../output/samples.npy", valid_samples)

    energy_values = np.ctypeslib.as_array(
        energy_out, shape=(config["num_chains"], config["num_MH"])
    )
