...
class Data(ctypes.Structure):
    _fields_ = [
        ...
        ("pLi", ctypes.POINTER(ctypes.c_double)), # prior cholesky inverse
        ("pL", ctypes.POINTER(ctypes.c_double)), # prior cholesky
...
def get_data():
    ...
    l = 2
    pSIGMA = sigma**2*np.exp(-np.abs(cs[None, :-1] - cs[:-1, None]) / l)
    print(pSIGMA)
    pL = np.linalg.cholesky(pSIGMA)
    print(pL)
    ...
    data = {
        ...
        "dc": H / N,
        "pm": 1.71472,
        "pLi": np.ascontiguousarray(pLi, dtype = np.float64),
        "pL": np.ascontiguousarray(pL, dtype = np.float64),
        "th": true_ages[0],
