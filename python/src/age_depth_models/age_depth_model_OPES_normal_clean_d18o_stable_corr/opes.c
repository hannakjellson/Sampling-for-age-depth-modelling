...
#pragma omp parallel num_threads(oc->hmcc->nch)
    {
        ...
        for (j = 0; j < d->N; j++)
        {
            variables[j] = oc->hmcc->sp[i * d->N + j];
        }
        ...
        for (j = 0; j < oc->hmcc->ns; j++)
        {
            ...
            for (k = 0; k < oc->nhmc; k++)
            {
                ...
                grad_energy_function(d, c14_depth_indices, c14_expected_ages, D18O_depth_indices, D18O_expected_ages, variables, sed_rates, gradient, d18o_gradient);
