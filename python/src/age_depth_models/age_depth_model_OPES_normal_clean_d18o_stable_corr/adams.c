...
void adams(ADAMConfig *ac, Data *d, double *energy_out, double *d18o_energy_out, double *samples_out)
{
...
#pragma omp parallel for
    for (i = 0; i < ac->nsp; i++)
    {
        ...
        for (j = 0; j < d->N; j++)
        {
            variables[j] = gsl_ran_gaussian(r, 1);
        }
        unnormalize_sed_rates(sed_rates, d->N, d->pm, d->pL, variables);
        ...
        for (j = 0; j < ac->mi; j++)
        {
            ...
            energy = energy_function(d, c14_expected_ages, D18O_expected_ages, variables, &d18o_energy);
            if (j % 10000 == 0)
            {
                printf("%d, %d, %f\n", i, j, energy);
            }
            ...
            stoch_grad_energy_function(d, num_d18O_points_stoch, random_indices_for_stoch_d18O, c14_depth_indices, c14_expected_ages, D18O_depth_indices, D18O_expected_ages, variables, sed_rates, gradient);
    ...
    for (i = 0; i < ac->nsp; i++)
    {
        ...
        d18o_energy_out[i] = temp[i].d18o_energy;
        for (l = 0; l < d->N; l++)
        {
