...
double get_CV_point(int N, double variables[N], gsl_vector *slow_direction)
{
    double sum = 0;
    for (int i = 0; i < N; i++)
    {
        sum += variables[i] * gsl_vector_get(slow_direction, i);
    }
    return sum;
}
...
void grad_bias(int N, double CV_point, double variables[N], int num_lambda, double gaussian_centers[num_lambda], double sigma, double sigma_2, double delta_F[num_lambda], int start_index, int end_index, double bias_gradient[N], gsl_vector *slow_direction)
{
    ...
    for (int i = 0; i < N; i++)
    {
        bias_gradient[i] = -variables[i] * gsl_vector_get(slow_direction, i) * sum_for_dV / sum_for_V;
    }
}
...
void hmc(int N, double H, double delta_c, const double *cs,
         double dt, int num_dt, int num_HMC, int num_samples, int num_chains, int cutoff,
         ...
         const double *D18O_depths, const double *D18O_sigma, const double *D18O_reference, const double *D18O_reference_times, double *samples_out)
{
        ...
        for (int l = 0; l < num_samples; l++)
        {
            for (int j = 0; j < num_HMC; j++)
            {
                ...
                for (int k = 0; k < num_dt; k++)
                {
                    ...
                    if (k != num_dt - 1)
                    {
                        for (int n = 0; n < N; n++)
                        {
                            momentum[n] -= dt * gradient[n];
                        }
                    }
                }
                for (int n = 0; n < N; n++)
                {
                    momentum[n] -= 0.5 * dt * gradient[n];
                }
            ...
            if (l >= cutoff)
            {
                ...
                for (int m = 0; m < N; m++)
                {
                    if (l >= cutoff)
                    {
                        samples_out[k * (num_samples - cutoff) * N + (l - cutoff) * N + m] = variables[m];
                    }
                }
            }
        }
