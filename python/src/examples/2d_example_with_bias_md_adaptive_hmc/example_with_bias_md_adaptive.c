...
double get_probability_estimate(double x, double *bias_centers, double *bias_heights, double *bias_widths, int bias_count, double *kernel_weights, double gamma, double beta, double sum_weights)
{
...
double get_probability_estimate_gradient(double x, double *bias_centers, double *bias_heights, double *bias_widths, int bias_count, double *kernel_weights, double gamma, double beta, double sum_weights)
{
    double dp = 0.0;
    for (int i = 0; i < bias_count; i++)
    {
...
double bias_potential(double x, double *bias_centers, double *bias_heights, double *bias_widths, int bias_count, double *kernel_weights, double gamma, double beta, double sum_weights, double Z, double DeltaE)
{
    double probability_estimate = get_probability_estimate(x, bias_centers, bias_heights, bias_widths, bias_count, kernel_weights, gamma, beta, sum_weights);
    double epsilon = exp(-beta * DeltaE / (1.0 - (1.0 / gamma)));
    double V = (1.0 - (1.0 / gamma)) * log(probability_estimate / Z + epsilon) / beta;
    return V;
}

void grad_bias(double x, double *bias_centers, double *bias_heights, double *bias_widths, int bias_count, double *kernel_weights, double gamma, double beta, double sum_weights, double Z, double DeltaE, double gradient[2])
{
    double probability_estimate = get_probability_estimate(x, bias_centers, bias_heights, bias_widths, bias_count, kernel_weights, gamma, beta, sum_weights);
    double probability_estimate_gradient = get_probability_estimate_gradient(x, bias_centers, bias_heights, bias_widths, bias_count, kernel_weights, gamma, beta, sum_weights);
    double epsilon = exp(-beta * DeltaE / (1.0 - (1.0 / gamma)));
...
void deposit_gaussian(double x, double width, double *bias_centers, double *bias_heights, double *bias_widths, double *kernel_weights, double current_weight, double *sum_squared_weights, int *bias_count, int iterations)
{
...
#pragma omp parallel for
    for (int k = 0; k < num_SP; k++)
    {
        ...
        {
            double potential = bias_potential(x_vec[0], bias_centers, bias_heights, bias_widths, bias_count, kernel_weights, gamma, beta, sum_weights, Z, DeltaE);
            ...
            { // 2 is the number of dimensions
                samples_out[2 * num_samples * k + 2 * j + l] = x_vec[l];
            }
            energy_out[k * num_samples + j] = energy_function(x_vec);
            bias_out[k * num_samples + j] = potential;
            ...
            for (int l = 0; l < num_HMC; l++)
            {
                ...
                double u_old = energy_function(x_vec) + bias_potential(x_vec[0], bias_centers, bias_heights, bias_widths, bias_count, kernel_weights, gamma, beta, sum_weights, Z, DeltaE);

                // compute gradient at old state
                double grad_energy_vec[2];
                grad_energy(x_vec, grad_energy_vec);
                double grad_bias_vec[2];
                grad_bias(x_vec[0], bias_centers, bias_heights, bias_widths, bias_count, kernel_weights, gamma, beta, sum_weights, Z, DeltaE, grad_bias_vec);
                ...
                double u_new = energy_function(x_vec) + bias_potential(x_vec[0], bias_centers, bias_heights, bias_widths, bias_count, kernel_weights, gamma, beta, sum_weights, Z, DeltaE);
