...
def define_c_types(lib):
    lib.md.argtypes = [
        ...
        # ctypes.POINTER(ctypes.c_double),    # bias_height
        # ctypes.POINTER(ctypes.c_double),    # bias_centers
...
def main():
    ...
    total = config["num_chains"] * config["num_MD"]
    ...
    # bias_heights = (ctypes.c_double * total)() # Trying to sample again using the bias function i have found.
    # bias_centers = (ctypes.c_double * total)() 
    ...
    print(len(samples))
    samples = np.reshape(samples, (config["num_chains"], config["num_MD"], 2))
    ...
    energy_values = np.reshape(energy_values, (config["num_chains"], config["num_MD"]))
    ...
    bias_weights = np.reshape(bias_weights, (config["num_chains"], config["num_MD"]))
    ...
    bias_std_out = np.reshape(bias_std_out, (config["num_chains"], config["num_MD"]))
