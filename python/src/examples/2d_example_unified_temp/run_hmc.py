...
def define_c_types(lib):
    lib.hmc.argtypes = [
        ...
        ctypes.c_double,  # estimate of energy expectation
...
def main():
    ...
    total_times_num_lambda = total * config["num_temps"]
    ...
    start_energies = start_energies[:, 90000:]
    energy_expectation = np.mean(start_energies, axis = (0,1))
    ...
    lib.hmc(
        ...
        ctypes.c_int(config["num_temps"]),
        ctypes.c_double(energy_expectation),
        samples_out,
        energy_out,
        delF_out,
        bias_out,
    ...
    np.save(f"../../../../output/2d_example_unified_temp/samples_{config_str}.npy", samples)
    ...
    np.save(f"../../../../output/2d_example_unified_temp/energy_values_{config_str}.npy", energy_values)

    delF_values = np.ctypeslib.as_array(delF_out)
    delF_values = np.reshape(delF_values, (config["num_chains"], config["num_samples"], config["num_temps"]))
    np.save(f"../../../../output/2d_example_unified_temp/delF_{config_str}.npy", delF_values)

    bias_values = np.ctypeslib.as_array(bias_out)
    bias_values = np.reshape(bias_values, (config["num_chains"], config["num_samples"]))
    np.save(f"../../../../output/2d_example_unified_temp/bias_values_{config_str}.npy", bias_values)
    ...
    np.save(f"../../../../output/2d_example_unified_temp/resampsamp_{config_str}.npy", resampled_samples)
