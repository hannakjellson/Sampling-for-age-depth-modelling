...
MAX_ENERGY = 15
ZERO_ENERGY = 0
ENERGIES = ((MAX_ENERGY)*(jnp.linspace(ZERO_ENERGY, MAX_ENERGY, NUM_ENERGIES) / MAX_ENERGY))
...
def bias_potential(energy, delta_F):
    ...
    energy_term = ((energy - ENERGIES) ** 2) / (2 * SIGMA**2)
    sum_for_V = jnp.sum(
            jnp.exp(-energy_term + delta_F)
        )
    return -jnp.log(sum_for_V / NUM_ENERGIES)
...
def update_delta_F(
    ...
    delta_F_nominator_sum += jnp.exp(
        -energy_term + potential * jnp.ones(NUM_ENERGIES)
    )

    delta_F_denominator_sum += jnp.exp(potential) * jnp.ones(NUM_ENERGIES)
...
def inference_loop(
    ...
    **adaptation_kwargs
    ...
    (
        ...
        energy=-logdensity,
        delta_F_nominator_sum=jnp.zeros(NUM_ENERGIES),
        delta_F_denominator_sum=jnp.zeros(NUM_ENERGIES),
        delta_F=jnp.zeros(NUM_ENERGIES),
        potential=0,
    )
