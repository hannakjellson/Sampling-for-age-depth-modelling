...
double bias_potential(double x, int num_lambda, double gaussian_centers[num_lambda], double sigma_2, double delta_F[num_lambda])
{
    double sum_for_V = 0;

    for (int i = 0; i < num_lambda; i++)
    {
        sum_for_V += exp(-(pow(x - gaussian_centers[i], 2) / (2 * sigma_2)) + delta_F[i]);
    }
    double V = -log(sum_for_V / num_lambda);
    return V;
}

void grad_bias(double x, int num_lambda, double gaussian_centers[num_lambda], double sigma, double sigma_2, double delta_F[num_lambda], double dV[2])
{
    double sum_for_dV = 0;
    double sum_for_V = 0;
    ...
    for (int i = 0; i < num_lambda; i++)
    {
        if (fabs(x - gaussian_centers[i]) > 3 * sigma)
        {
            continue;
        }
        double gaussian_diff = (x - gaussian_centers[i]);
    ...
    dV[0] = sum_for_dV / sum_for_V;
    dV[1] = 0;
}

void update_delta_F(double x, int lambda_index, int num_lambda, double sigma_2, double gaussian_centers[num_lambda], double delta_F_nominator_sum[num_lambda], double delta_F_denominator_sum[num_lambda], double delta_F[num_lambda], double potential)
{
    delta_F_nominator_sum[lambda_index] += exp((-pow(x - gaussian_centers[lambda_index], 2) / (2 * sigma_2)) + potential);
         ...
         double *samples_out, double *energy_out, double *delta_F_out, double *bias_out)
{
    ...
    for (int i = 0; i < num_lambda; i++)
    {
        gaussian_centers[i] = -3 + 6 * ((double)i / (num_lambda - 1));
    }
...
#pragma omp parallel for
    for (int k = 0; k < num_SP; k++)
    {
        ...
        for (int i = 0; i < num_lambda; i++)
        {
            delta_F_nominator_sum[i] = exp(-pow(x_vec[0] - gaussian_centers[i], 2) / (2 * sigma_2));
        ...
        for (int s = 0; s < num_samples; s++)
        {
            ...
            for (int j = 0; j < num_HMC; j++)
            {
                ...
                double u_old = energy_function(x_vec) + bias_potential(x_old, num_lambda, gaussian_centers, sigma_2, delta_F);

                // compute gradient at old state
                double grad_energy_vec[2];
                grad_energy(x_vec, grad_energy_vec);
                double grad_bias_vec[2];
                grad_bias(x_vec[0], num_lambda, gaussian_centers, sigma, sigma_2, delta_F, grad_bias_vec);
                ...
                double u_new = energy_function(x_vec) + bias_potential(x_vec[0], num_lambda, gaussian_centers, sigma_2, delta_F);
            ...
            bias_out[k * num_samples + s] = bias_potential(x_vec[0], num_lambda, gaussian_centers, sigma_2, delta_F);
            for (int l = 0; l < 2; l++)
            ...
            for (int i = 0; i < num_lambda; i++)
            {
                update_delta_F(x_vec[0], i, num_lambda, sigma_2, gaussian_centers, delta_F_nominator_sum, delta_F_denominator_sum, delta_F, bias_out[k * num_samples + s]);
            }
        }
    }
}